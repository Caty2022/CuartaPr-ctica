...
class UserController {
  ...
  async cambiarRolPremium(req, res) {
    ...
    try {
      ...
      const uploadedDocuments = user.documents.map(
        (doc) => doc.name.split(".")[0]
      ); // Extraer el nombre del documento sin la extensión
      ...
      if (missingDocuments.length > 0) {
        return res
          .status(400)
          .json({
            message: `Debe cargar los documentos requeridos antes de actualizar a premium: ${missingDocuments.join(
              ", "
            )}`,
          });
      }
  ...
  async subirDocumentos(req, res) {
    ...
    try {
        ...
        if (uploadedDocuments.document) {
          uploadedDocuments.document.forEach((doc) => {
            ...
            if (doc.originalname.includes("Identificacion")) {
              ...
              doc.originalname.includes("Comprobante de estado de cuenta")
            ) {
              documentType = "Comprobante de estado de cuenta";
            }

            const normalizedFileName = `${documentType}_${uid}_${Date.now()}${path.extname(
              doc.originalname
            )}`;
      ...
      const missingDocuments = requiredDocuments.filter(
        (doc) => !uploadedDocumentNames.includes(doc)
      );
